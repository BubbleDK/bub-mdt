import type { Call, Unit, UnitsObject } from "../typings";
...
export const convertCalls = (
    resp: { [key: string]: CallsResponse } | CallsResponse[]
): Call[] => {
    if (Array.isArray(resp))
        return resp.map((entry) => ({
            ...entry,
            units: convertUnitsToArray(entry.units),
        }));
