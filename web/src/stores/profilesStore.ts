...
const useProfilesStore = create<ProfilesStore>((set) => ({
    ...
    getProfiles: async (): Promise<{ profiles: PartialProfileData[] }> => {
        ...
        return await fetchNui<{ profiles: PartialProfileData[] }>(
            "getAllProfiles"
        );
    },
    setSelectedProfile: (profile: Profile | null) => {
        if (isEnvBrowser())
            set({
                selectedProfile: DEBUG_PROFILE,
                profileCards: DEBUG_PROFILECARDS,
            });

        set({ selectedProfile: profile });
    },
    setProfileCards: (cards: CustomProfileData[]) =>
        set({ profileCards: cards }),
