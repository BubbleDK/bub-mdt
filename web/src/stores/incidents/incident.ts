...
import type { Criminal, Evidence, Incident, Officer } from "../../typings";
...
interface IncidentStoreState {
    ...
    updateIncidentField: <K extends keyof Report>(
        field: K,
        value: Report[K]
    ) => void;
    ...
    setCriminals: (
        update: Criminal[] | ((prevCriminals: Criminal[]) => Criminal[])
    ) => void;
    setCriminal: (
        citizenid: string,
        update: Criminal | ((prevCriminal: Criminal) => Criminal)
    ) => void;
    setOfficersInvolved: (
        update: Officer[] | ((prevOfficers: Officer[]) => Officer[])
    ) => void;
    setEvidence: (
        update: Evidence[] | ((prevEvidence: Evidence[]) => Evidence[])
    ) => void;
}
            ...
            incident: {
                ...
                criminals:
                    typeof update === "function"
                        ? update(state.incident.criminals)
                        : update,
            },
        }));
    },
    ...
    setOfficersInvolved: (update) => {
        set((state) => ({
            incident: {
                ...
                officersInvolved:
                    typeof update === "function"
                        ? update(state.incident.officersInvolved)
                        : update,
            },
        }));
    },
    ...
    setEvidence: (update) => {
        set((state) => ({
            incident: {
                ...
                evidence:
                    typeof update === "function"
                        ? update(state.incident.evidence)
                        : update,
            },
        }));
    },
