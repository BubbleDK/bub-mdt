...
import type { PartialIncidentData } from "../../typings/incident";
...
interface IncidentsStoreState {
    ...
    setIncidents: (
        update:
            | PartialIncidentData[]
            | ((prevIncidents: PartialIncidentData[]) => PartialIncidentData[])
    ) => void;
}
...
const getIncidents = async (): Promise<{
    incidents: PartialIncidentData[];
}> => {
    if (isEnvBrowser()) {
        return {
...
const useIncidentsStore = create<IncidentsStoreState>((set) => ({
    ...
    setIncidents: (update) => {
        set((state) => ({
            incidents:
                typeof update === "function" ? update(state.incidents) : update,
        }));
    },
}));
