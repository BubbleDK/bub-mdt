...
interface ChargesStoreState {
	...
	setSelectedCharges: (
		update: SelectedCharge[] | ((prev: SelectedCharge[]) => SelectedCharge[])
	) => void;
	setSelectedCharge: (
		label: string,
		update: (charge: SelectedCharge) => SelectedCharge
	) => void;
}
...
const useSelectedChargesStore = create<ChargesStoreState>((set) => ({
	...
	setSelectedCharges: (update) =>
		set((state) => ({
			selectedCharges:
				typeof update === "function" ? update(state.selectedCharges) : update,
		})),
