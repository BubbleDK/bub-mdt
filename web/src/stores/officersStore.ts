...
import type { Officer } from "../typings";
...
const useOfficerStore = create<OfficerStore>((set) => ({
    ...
    addOfficer: (officer) =>
        set((state) => ({ officers: [...state.officers, officer] })),
    removeOfficer: (citizenid) =>
        set((state) => ({
            officers: state.officers.filter((o) => o.citizenid !== citizenid),
        })),
}));
