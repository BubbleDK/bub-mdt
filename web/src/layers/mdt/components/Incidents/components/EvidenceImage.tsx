...
const EvidenceImage: React.FC<{
	evidence: { label: string; image: string };
}> = ({ evidence }) => {
			...
			>
				<Image
					...
					onClick={() =>
						modals.open({
							children: (
								<Image src={evidence.image} onClick={() => modals.closeAll()} />
							),
							...
							modals.openConfirmModal({
								...
								onConfirm: async () => {
									...
									setEvidence((prev) =>
										prev.filter(
											(prevEvidence) =>
												prevEvidence.image !== evidence.image &&
												prevEvidence.label !== evidence.label
										)
									);
								},
							})
