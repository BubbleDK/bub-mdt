...
const AddEvidenceModal: React.FC = () => {
	...
	const form = useForm({
		...
		validate: {
			label: (value) =>
				value.length === 0 ? locales.image_label_required : null,
			image: (value) =>
				value.length === 0 ? locales.image_url_required : null,
		},
	});
	...
	const handleSubmit = async (values: { label: string; image: string }) => {
		...
		await fetchNui(
			"addEvidence",
			{ id: incident.id, evidence: { ...values } },
			{ data: 1 }
		);
								...
								<ActionIcon
									...
									onClick={async () => {
										await fetchNui(
											...
											{
												id: incident.id,
												imageLabel: form.getInputProps("label").value,
												type: "incident",
											},
											{ data: 1 }
										);
