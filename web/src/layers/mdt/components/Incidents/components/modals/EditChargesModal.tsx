...
const EditChargesModal = ({ criminal }: Props) => {
	...
	useEffect(() => {
		if (debouncedSearchQuery.trim() === "") {
			...
			const categories: ChargesObject = Object.entries(charges).reduce(
				(acc, [category, chargesArray]) => {
					const filteredCharges = debouncedSearchQuery.trim()
						? chargesArray.filter((charge) =>
								charge.label
									.toLowerCase()
											...
											<div key={category}>
												...
												<div className='edit-charges-modal-charges-content'>
													{chargesArray.map((charge, index) => (
														<div
															className='edit-charges-modal-charge-card'
															key={index}
														>
															<Text
																fz='sm'
															...
															<Group position='apart'>
																<Badge
																	...
																	color={
																		charge.type.toLocaleLowerCase() === "felony"
																			? "red"
																			: charge.type.toLocaleLowerCase() ===
																			  "infraction"
																			? "green"
																			: "yellow"
																	}
																	...
																	<ActionIcon
																		onClick={() =>
																			setSelectedCharges((prev) => {
																				const prevChargeIndex = prev.findIndex(
																					(el) => el.label === charge.label
																				);
