				...
				<div className='incident-criminal-card-buttons'>
								...
								modals.openConfirmModal({
									...
									onConfirm: async () => {
										...
										if (success)
											setCriminals((prev) =>
												prev.filter(
													(crim) => crim.citizenid !== criminal.citizenid
												)
											);
									},
									children: (
										<Text size='sm' c='dark.2'>
											{locales.remove_criminal_confirm.format(
												criminal.firstname,
												criminal.lastname
											)}
										</Text>
					...
					>
						<ActionIcon
							...
							onClick={async () => {
								if (!isEnvBrowser()) {
									const resp = await fetchNui<Profile>(
										"getProfile",
										criminal.citizenid
									);
									...
									"saveCriminal",
									{
										...
										criminal: {
											...
											warrantExpiry: criminal.warrantExpiry
												? dayjs(criminal.warrantExpiry).format(
														"YYYY-MM-DD HH:mm:ss"
												  )
												: null,
										},
			...
			<div className='incident-criminal-content'>
				<div className='incident-criminal-edit-charges-badges'>
					<Badge
						...
						onClick={() => {
							...
							modals.open({
								...
								styles: {
									body: { height: 700, overflow: "hidden" },
									content: { width: 1400 },
								},
								centered: true,
								size: 1445,
								children: <EditChargesModal criminal={criminal} />,
							});
						}}
					>
						{locales.edit_charges}
					...
					{criminal.charges.map((charge, index) => (
						<Badge
							...
							color={
								charge.type.toLowerCase() === "felony"
									? "red"
									: charge.type.toLowerCase() === "infraction"
									? "green"
									: "yellow"
							}
							key={index}
				...
				<Checkbox
					...
					onChange={() =>
						setCriminal(criminal.citizenid, (prev) => ({
							...prev,
							issueWarrant: !prev.issueWarrant,
						}))
					}
				/>
				...
				{criminal.issueWarrant ? (
					<DatePickerInput
						...
						minDate={new Date()}
						value={
							criminal.warrantExpiry ? new Date(criminal.warrantExpiry) : null
						}
						onChange={(val) => {
							setCriminal(criminal.citizenid, (prev) => ({
								...prev,
					...
					<>
						<Select
							...
							value={
								criminal.penalty.reduction
									? criminal.penalty.reduction.toString()
									: null
							}
						...
						<Group>
							<Checkbox
								...
								onChange={() =>
									setCriminal(criminal.citizenid, (prev) => ({
										...prev,
										pleadedGuilty: !prev.pleadedGuilty,
									}))
								}
							/>
							<Checkbox
