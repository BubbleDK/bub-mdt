...
const AddOfficerModal: React.FC = () => {
  const { report, setOfficersInvolved } = useReportStore();
  ...
  const handleSubmit = async (officer: Officer) => {
    if (report.officersInvolved.some(o => o.citizenid === officer.citizenid)) return modals.closeAll();

    await fetchNui('addReportOfficer', { id: report.id, citizenid: officer.citizenid }, { data: 1 });
    modals.closeAll();
