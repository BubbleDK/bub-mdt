...
const SORT_OPTIONS: SelectOption[] = [
    {
        label: "Newest first",
        value: "newest",
        description: "Most recently filed cases",
    },
    {
        label: "Oldest first",
        value: "oldest",
        description: "Earliest filed cases",
    },
    {
        label: "Highest case number",
        value: "number",
        description: "Descending incident number",
    },
];
        ...
        >
                ...
                <div className="relative mb-6 flex items-end justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className="flex h-9 items-center gap-2 rounded-lg bg-blue-500 px-3.5 text-xs font-semibold text-white shadow-lg shadow-blue-950/20 transition hover:bg-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                        >
                    ...
                    <AnimatePresence>
                        {hasRefinements && (
                            <motion.button
                                ...
                                className="ml-1 flex h-9 items-center gap-1.5 rounded-lg px-2.5 text-xs text-neutral-500 transition hover:bg-white/[0.05] hover:text-neutral-200"
                            >
            ...
            <section className="min-h-0 flex-1 overflow-y-auto px-7 py-5">
                <div className="mb-3 flex items-center justify-between text-[11px] font-semibold uppercase tracking-[0.14em] text-neutral-500">
                    <span>
                        {search || activeFilterCount
                            ? `${incidents.length} matching cases`
                            : "Recent case files"}
                    </span>
                    <span>
                        {sortOrder === "oldest"
                ...
                {query.isLoading ? (
                    ...
                    <EmptyState
                        title="Case files unavailable"
                        body="The incident database could not be reached."
                        action={() => query.refetch()}
                    />
                ) : incidents.length === 0 ? (
                    <EmptyState
                        title="No matching incidents"
                        body="Try changing your search or clearing one of the active filters."
                        action={hasRefinements ? resetFilters : undefined}
                        ...
                        {incidents.map((incident) => (
                            ...
                            >
                                ...
                                <div className="min-w-0">
                                    <h2 className="truncate text-sm font-medium text-neutral-100 group-hover:text-white">
                                        {incident.title}
                                    </h2>
    ...
    return (
        <div className="flex h-64 flex-col items-center justify-center text-center">
            <div className="mb-4 rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
                <FileSearch className="h-7 w-7 text-neutral-500" />
            </div>
            ...
            {action && (
                <button
                    onClick={action}
                    className="mt-4 rounded-lg bg-blue-500 px-4 py-2 text-xs font-semibold text-white hover:bg-blue-400"
                >
                    {actionLabel}
                </button>
            )}
    ...
    return (
        <div className="space-y-2.5">
            {Array.from({ length: 6 }, (_, i) => (
                <div
                    key={i}
                    className="h-[74px] animate-pulse rounded-xl border border-white/[0.05] bg-white/[0.025]"
                />
            ))}
        </div>
    );
