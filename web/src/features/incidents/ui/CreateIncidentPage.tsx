...
export function CreateIncidentPage() {
    ...
    const addEvidence = () => {
        ...
        if (!result.evidence) {
            setError(result.error ?? "Invalid evidence.");
            return;
        }
        setEvidence((current) => [...current, result.evidence!]);
        setDraft(EMPTY_EVIDENCE);
        setError("");
    };
    ...
    return (
        ...
        >
            <form onSubmit={submit} className="min-h-0 flex-1 overflow-y-auto">
                <div className="mx-auto max-w-5xl p-6 pb-28">
                    ...
                    <div className="incident-create-grid grid grid-cols-[minmax(0,1.25fr)_minmax(310px,0.75fr)] items-start gap-5">
                        <div className="space-y-5">
                            <Section
                                ...
                                description="Use a specific title that is easy to recognise in search results."
                            >
                                <label className="block">
                                    ...
                                    <input
                                        ...
                                        placeholder="e.g. Armed robbery — Vespucci Boulevard"
                                        className="h-11 w-full rounded-lg border border-white/[0.09] bg-black/10 px-3.5 text-sm text-white outline-none placeholder:text-neutral-600 focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/10"
                                    />
                                    <span className="mt-1.5 block text-right text-[10px] text-neutral-600">
                                        {title.length}/120
                                    </span>
                                </label>
                            </Section>
                            <Section
                                ...
                                description="Record the sequence of events and actions taken by responding units."
                            >
                                <RichTextEditor
                                    content={narrative}
                                    onChange={setNarrative}
                                    height={285}
                                    placeholder="Document what happened, where units responded, and the current case status..."
                                />
                            </Section>
                        </div>
                        <Section
                            ...
                            description="Optionally attach known image references now."
                        >
                            <div className="space-y-2.5">
                                ...
                                <button
                                    type="button"
                                    onClick={addEvidence}
                                    className="flex h-10 w-full items-center justify-center gap-2 rounded-lg border border-white/[0.09] bg-white/[0.04] text-xs font-medium text-neutral-300 hover:bg-white/[0.08] hover:text-white"
                                >
                            ...
                            <div className="mt-4 space-y-2">
                                {evidence.map((item) => (
                                    <div
                                        ...
                                        className="flex items-center gap-3 rounded-lg border border-white/[0.065] bg-black/10 p-2"
                                    >
                                        <img
                                            src={item.image}
                                            alt=""
                                ...
                                {!evidence.length && (
                                    <div className="flex flex-col items-center rounded-lg border border-dashed border-white/[0.08] py-7">
                ...
                <footer className="sticky bottom-0 flex min-h-20 flex-wrap items-center justify-between gap-3 border-t border-white/[0.07] bg-brand-dark/95 px-4 py-3 backdrop-blur-xl">
                    ...
                    <div className="flex gap-2">
                        <button
                            type="button"
                            onClick={() => navigate(-1)}
                            className="flex h-10 items-center gap-2 rounded-lg px-4 text-xs font-medium text-neutral-400 hover:bg-white/[0.05] hover:text-white"
                        >
                            <X className="h-4 w-4" />
                            Cancel
                        </button>
                        <button
                            type="submit"
                            disabled={isSaving}
                            className="flex h-10 min-w-36 items-center justify-center gap-2 rounded-lg bg-blue-500 px-5 text-xs font-semibold text-white hover:bg-blue-400 disabled:opacity-50"
                        >
                            {isSaving ? (
