...
const ReportRight = () => {
	const { report, isReportActive, setOfficersInvolved, setCitizensInvolved } =
		useReportStore();
										...
										onClick={() => {
											modals.openConfirmModal({
												...
												onConfirm: async () => {
													await fetchNui(
														"removeReportOfficer",
														{ id: report.id, citizenid: officer.citizenid },
														{ data: 1 }
													);
									...
									<ActionIcon
										...
										onClick={() => {
											modals.openConfirmModal({
												...
												children: (
													<Text size='sm' c='dark.2'>
														{locales.remove_citizen_confirm.format(
															citizen.firstname,
															citizen.lastname
														)}
													</Text>
												...
												onConfirm: async () => {
													...
													setCitizensInvolved((prev) =>
														prev.filter(
															(prevCitizen) =>
																prevCitizen.citizenid !== citizen.citizenid
														)
													);
												},
											});
