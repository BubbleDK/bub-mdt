...
const BoloModal = (props: props) => {
	const { setSelectedVehicle, setIsVehicleBOLO, setBOLOExpirationDate } =
		useVehiclesStore();
			...
			>
				<Button
					...
					onClick={async () => {
						...
						const resp = await fetchNui<Vehicle>(
							"getVehicle",
							{ plate: props.bolo.plate },
							{
								data: {
									...DEBUG_VEH,
								},
						...
						const isVehicleBolo = await fetchNui<boolean>(
							"isVehicleBOLO",
							{ plate: props.bolo.plate },
							{
								data: false,
						...
						if (isVehicleBolo) {
							const BOLODate = await fetchNui<string>(
								"getBOLOExpirationDate",
								{ plate: props.bolo.plate },
								{
									data: "12/12/2024",
								}
