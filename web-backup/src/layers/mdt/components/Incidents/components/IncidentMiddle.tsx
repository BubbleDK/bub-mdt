									...
									onClick={() => {
										modals.openConfirmModal({
											...
											onConfirm: async () => {
												...
												if (success)
													setIncidents((prev) =>
														prev.filter((record) => record.id !== incident.id)
													);
											...
											children: (
												<Text size='sm' c='dark.2'>
													{locales.delete_incident_confirm.format(
														incident.title
													)}
												</Text>
											),
											...
											modals.openConfirmModal({
												title: (
													<Text
														...
														weight={500}
													>
														{locales.remove_officer}
													</Text>
												),
												centered: true,
												children: (
													<Text size='sm' c='dark.2'>
														{locales.remove_officer_confirm.format(
															officer.firstname,
															officer.lastname,
															officer.callsign
														)}
												...
												onConfirm: async () => {
													await fetchNui(
														"removeOfficer",
														{ id: incident.id, citizenid: officer.citizenid },
														{ data: 1 }
													);
													setOfficersInvolved((prev) =>
														prev.filter(
															(prevOfficer) =>
																prevOfficer.citizenid !== officer.citizenid
														)
													);
												},
											});
				...
				<div className='card-background profile-card-info'>
					...
					<div className='profile-card-info-badges'>
						<Button
							onClick={() =>
								modals.open({
									centered: true,
									title: (
										<Text style={{ fontSize: 16, color: "white" }} weight={500}>
											{locales.add_evidence}
										</Text>
									),
									children: <AddEvidenceModal />,
									size: "sm",
								})
							}
							variant='filled'
						...
						{incident.evidence.map((evidence) => (
							<EvidenceImage
								evidence={evidence}
								key={`${evidence.image}-${evidence.label}`}
							/>
						))}
					</div>
				</div>
