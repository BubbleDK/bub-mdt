...
interface ReportStoreState {
	...
	setCitizensInvolved: (
		update:
			| PartialProfileData[]
			| ((prevProfiles: PartialProfileData[]) => PartialProfileData[])
	) => void;
	setEvidence: (
			...
			report: {
				...
				officersInvolved:
					typeof update === "function"
						? update(state.report.officersInvolved)
						: update,
			},
		}));
	},
	...
	setCitizensInvolved: (update) => {
		set((state) => ({
			report: {
				...
				citizensInvolved:
					typeof update === "function"
						? update(state.report.citizensInvolved)
						: update,
			},
		}));
	},
	...
	setEvidence: (update) => {
		set((state) => ({
			report: {
				...state.report,
				evidence:
					typeof update === "function" ? update(state.report.evidence) : update,
			},
		}));
	},
}));
